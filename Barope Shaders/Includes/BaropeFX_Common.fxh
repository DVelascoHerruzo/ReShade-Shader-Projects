...
#define BFX_PI 3.14159265358979323846f

namespace Common {

    // Common Buffers
    texture2D BaropeBufferTex { Width = BUFFER_WIDTH; Height = BUFFER_HEIGHT; Format = RGBA16F; };
    sampler2D BaropeBuffer { Texture = BaropeBufferTex; MagFilter = POINT; MinFilter = POINT; MipFilter = Point; };
    sampler2D BaropeBufferLinear { Texture = BaropeBufferTex; };
    storage2D s_BaropeBuffer { Texture = BaropeBufferTex; };

    // Miscellanous Buffers
    //Mirror Buffer
    sampler2D BaropeBufferMirror { Texture = BaropeBufferTex; AddressU = MIRROR; AddressV = MIRROR; AddressW = MIRROR; MagFilter = POINT; MinFilter = POINT; MipFilter = Point; };
    sampler2D BaropeBufferMirrorLinear { Texture = BaropeBufferTex; AddressU = MIRROR; AddressV = MIRROR; AddressW = MIRROR; };

    //Wrap Buffer
    sampler2D BaropeBufferWrap { Texture = BaropeBufferTex; AddressU = WRAP; AddressV = WRAP; AddressW = WRAP; MagFilter = POINT; MinFilter = POINT; MipFilter = Point; };
    sampler2D BaropeBufferWrapLinear { Texture = BaropeBufferTex; AddressU = WRAP; AddressV = WRAP; AddressW = WRAP; };

    //Repeat Buffer
    sampler2D BaropeBufferRepeat { Texture = BaropeBufferTex; AddressU = REPEAT; AddressV = REPEAT; AddressW = REPEAT; MagFilter = POINT; MinFilter = POINT; MipFilter = Point; };
    sampler2D BaropeBufferRepeatLinear { Texture = BaropeBufferTex; AddressU = REPEAT; AddressV = REPEAT; AddressW = REPEAT; };

    //Border Buffer
    sampler2D BaropeBufferBorder { Texture = BaropeBufferTex; AddressU = BORDER; AddressV = BORDER; AddressW = BORDER; MagFilter = POINT; MinFilter = POINT; MipFilter = Point; };
    sampler2D BaropeBufferBorderLinear { Texture = BaropeBufferTex; AddressU = BORDER; AddressV = BORDER; AddressW = BORDER; };
    ...
    float3 WhiteBalance(float3 col, float temp, float tint){
        ...
        float x = 0.31271f - t1 * (t1 < 0 ? 0.1 : 0.05);
...
// -----------------------------------------------Color Space Convertors-------------------------------------------------
    ...
    float3 convertXYZtoYxy(float3 col) {
        float inv = 1.0f / dot(col, 1.0f);

        return float3(col.y, col.x * inv, col.y * inv);
    }

    float3 convertRGBtoYxy(float3 col) {
        return convertXYZtoYxy(convertRGBtoXYZ(col));
    }

    float3 convertXYZtoRGB(float3 col) {
        float3 rgb;
    ...
    float3 convertYxytoXYZ(float3 col){
        float3 xyz;
    ...
    float3 convertYxytoRGB(float3 col) {
        return convertXYZtoRGB(convertYxytoXYZ(col));
    }
    ...
    float3 RGBtoHSL(in float3 RGB) {
        ...
        float S = HCV.y / (1 - abs(L * 2 - 1) + BFX_EPSILON);
...
// Reshade Depth Mult
    ...
    #define RESHADE_DEPTH_MULTIPLIER 1.0f
#endif
...
namespace ReShade
{
    ...
    float GetLinearDepth(float2 texcoord, float zoom)
    {
#if RESHADE_DEPTH_INPUT_IS_UPSIDE_DOWN
        texcoord.y = 1.0 - texcoord.y;
#endif
