...
#ifndef BFX_DEBUG_MASK
 #define BFX_DEBUG_MASK 0
#endif

#ifndef BFX_DEBUG_SPANS
 #define BFX_DEBUG_SPANS 0
#endif

#ifndef BFX_HORIZONTAL_SORT
 #define BFX_HORIZONTAL_SORT 0
#endif
...
texture2D BFX_PixelSortMaskTex { Width = BUFFER_WIDTH; Height = BUFFER_HEIGHT; Format = R8; }; 
sampler2D Mask { Texture = BFX_PixelSortMaskTex; };
storage2D s_Mask { Texture = BFX_PixelSortMaskTex; };

texture2D BFX_SortValueTex { Width = BUFFER_WIDTH; Height = BUFFER_HEIGHT; Format = R8; }; 
sampler2D SortValue { Texture = BFX_SortValueTex; };
storage2D s_SortValue { Texture = BFX_SortValueTex; };

texture2D BFX_SpanLengthsTex { Width = BUFFER_WIDTH; Height = BUFFER_HEIGHT; Format = R16F; }; 
sampler2D SpanLengths { Texture = BFX_SpanLengthsTex; };
storage2D s_SpanLengths { Texture = BFX_SpanLengthsTex; };

sampler2D PixelSort { Texture = BFXTemp1::BFX_RenderTex1; MagFilter = POINT; MinFilter = POINT; MipFilter = POINT; };
...
void CS_CreateMask(uint3 id : SV_DISPATCHTHREADID) {
    ...
    float4 col = saturate(tex2Dlod(ReShade::BackBuffer, float4(uv, 0, 0)));
...
void CS_CreateSortValues(uint3 id : SV_DISPATCHTHREADID) {
    float4 col = tex2Dfetch(ReShade::BackBuffer, id.xy);
...
void CS_ClearBuffers(uint3 id : SV_DISPATCHTHREADID) {
    ...
    tex2Dstore(BFXTemp1::s_RenderTex, id.xy, 0);
}
...
void CS_VisualizeSpans(uint3 id : SV_DISPATCHTHREADID) {
    ...
    if (spanLength >= 1) {
        ...
        for (int i = 0; i < spanLength; ++i) {
            ...
            tex2Dstore(BFXTemp1::s_RenderTex, idx, c);
        }
    }
}
...
void CS_PixelSort(uint3 id : SV_DISPATCHTHREADID) {
    ...
    if (spanLength >= 1) {
        ...
        for (uint i = 0; i < (spanLength / 2) + 1; ++i) {
            ...
            tex2Dstore(BFXTemp1::s_RenderTex, minIdx, pow(abs(tex2Dfetch(ReShade::BackBuffer, minColorIdx)), _SortedGamma));
            tex2Dstore(BFXTemp1::s_RenderTex, maxIdx, pow(abs(tex2Dfetch(ReShade::BackBuffer, maxColorIdx)), _SortedGamma));
...
void CS_Composite(uint3 id : SV_DISPATCHTHREADID) {
    if (tex2Dfetch(Mask, id.xy).r == 0) {
        tex2Dstore(BFXTemp1::s_RenderTex, id.xy, tex2Dfetch(ReShade::BackBuffer, id.xy));
    }
}

technique BFX_PixelSort < ui_label = "Pixel Sort"; ui_tooltip = "(EXTREMELY HIGH PERFORMANCE COST) Sort the game pixels."; > {
    pass {
...
#if BFX_DEBUG_MASK != 0
    pass {
        RenderTarget = ReShade::BackBuffer;
